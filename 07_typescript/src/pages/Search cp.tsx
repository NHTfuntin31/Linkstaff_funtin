...
const Search = () => {
	...
	const newText = searchFilterChange
	const { isLoading, error, data } = useQuery(['jobData'], () => {
  if (searchText === '') {
    return fetch('https://lusty.asia:1443/api/jobs')
      .then(res => res.json());
  } else {
    return fetch(`https://lusty.asia:1443/api/jobs?filters[description][$contains]=${newText}`)
      .then(res => res.json());
  }
});
  ...
  const handleSearchTextChange = (e: any) => {
	...
	const abc = async (e: any) => {
		...
		// Continue with updating the UI after dispatch is completed
		// updateUI(newValue);
	};
